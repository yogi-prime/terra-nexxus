...
export const QuickFilters = () => {
  ...
  const goToType = (typeName: string) => {
    const params = new URLSearchParams({ property_type: typeName });
    nav({ pathname: "/marketplace", search: params.toString() });
  };
  ...
  if (loading) {
    return (
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          {/* centered skeleton grid */}
          <div className="mx-auto max-w-6xl grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
            {Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                className="h-36 animate-pulse bg-card border border-border rounded-xl"
              />
            ))}
          </div>
        </div>
  ...
  return (
    <section className="py-16 bg-background">
      <div className="container mx-auto px-4">
        ...
        {/* CENTERED GRID */}
        <div className="mx-auto max-w-6xl grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5 justify-items-center">
          {propertyTypes.map((type) => {
            const IconComponent = getIconFromString(type.name);
            return (
              <button
                ...
                className="group w-full max-w-[220px] bg-card border border-border rounded-xl p-6 text-center hover:border-primary hover:shadow-lg transition-all duration-300 hover:-translate-y-2 focus:outline-none focus:ring-2 focus:ring-primary"
                aria-label={`View ${type.name} properties`}
              >
                <div className="flex justify-center mb-4">
                  <div className="p-4 bg-white border border-primary rounded-full group-hover:bg-primary group-hover:text-white flex items-center justify-center transition-colors duration-300">
                    <IconComponent className="h-8 w-8 text-primary group-hover:text-white transition-colors" />
