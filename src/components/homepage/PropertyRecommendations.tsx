...
// };
// import { useEffect, useState } from "react";
// import { Heart, MapPin, Shield, Star, Eye } from "lucide-react";
// import { Button } from "@/components/ui/button";
// import { Badge } from "@/components/ui/badge";
// import { Card, CardContent } from "@/components/ui/card";
...
//                 {/* ✅ View Details button -> navigate to /property-show/:id */}
//                 <Button
//                   size="sm"
//                   className="w-full"
//                   onClick={() => navigate(`/property-show/${property.id}`)}
//                 >
...
/** -------- INR short formatter: 10k, lakh, cr (with ₹) -------- */
function formatINRShort(n?: number | string | null): string {
  ...
  if (abs >= 1e7) {
    const val = num / 1e7;
    const d = val < 10 ? 2 : val < 100 ? 1 : 0; // nicer precision
    return `₹${val.toFixed(d).replace(/\.0+$/, "")} cr`;
  }
  // Lakh
  if (abs >= 1e5) {
    const val = num / 1e5;
    const d = val < 10 ? 2 : val < 100 ? 1 : 0;
    return `₹${val.toFixed(d).replace(/\.0+$/, "")} lakh`;
  }
  // Thousands (normal Indian commas)
...
function mapToProperty(p: BackendProperty): Property {
  ...
  // Build price string with short INR format
  let priceStr = "Price on request";
  if (priceMin && priceMax) {
    priceStr = `${formatINRShort(priceMin)} - ${formatINRShort(priceMax)}`;
  } else if (priceMin) {
    priceStr = formatINRShort(priceMin);
  } else if (priceMax) {
    priceStr = formatINRShort(priceMax);
  }
  ...
  return {
    ...
    price: priceStr,
    originalPrice: actual ? formatINRShort(actual) : null,
    bhk: bedrooms ? `${bedrooms} BHK` : "Studio",
    area: p?.layout?.size_text || p?.size_sqft_text || "N/A",
    verified: !!(p?.legal?.noc_available ?? p?.noc_available ?? false),
