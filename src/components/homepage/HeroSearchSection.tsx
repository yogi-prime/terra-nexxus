...
export const HeroSearchSection = () => {
  ...
  // Navigate to marketplace with URL params
  ...
  const goWithLocality = (locality: string) => {
    const params = new URLSearchParams();
    params.set("city", locality);
    nav({ pathname: "/marketplace", search: params.toString() });
  };
          ...
          <div className="rounded-2xl border border-white/10 bg-white/90 backdrop-blur-md shadow-[0_20px_60px_rgba(0,0,0,0.25)] p-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-2 relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500" />
                <Input
                  placeholder="Search by property name, type, price, e.g. '4BHK', 'villa'..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && goToMarketplace()}
                  className="h-14 pl-12 bg-white border-white/30 text-base"
                />
              </div>
              ...
              <Button
                size="lg"
                className="h-14 px-8 shadow-lg shadow-emerald-500/25 bg-gradient-to-r from-emerald-500 to-lime-400 hover:from-emerald-600 hover:to-lime-500 text-white"
        ...
        <div className="text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {cities.slice(0, 6).map((locality) => (
              <button
                key={locality}
                className="px-4 py-2 rounded-full text-sm font-medium bg-white/15 text-white border border-white/20 hover:bg-white/25 transition-colors"
                onClick={() => goWithLocality(locality)}
              >
                {locality}
              </button>
            ))}
