...
interface Property {
  ...
  developer: string;
  location: string;
  price: string;          // formatted text we render
  image: string;
  bhk: string;
  area: string;
  rating: number;
...
// format 12345 -> ₹12,345 ; 150000 -> ₹1.5 L ; 12300000 -> ₹1.23 Cr
function formatINRCompact(raw: any): string {
  const n = Number(raw);
  ...
  // below 1 lakh -> full number with Indian commas
    ...
    const fetchProperties = async () => {
      ...
      try {
        const res = await fetch(
          `https://app.terranexxus.com/api/v1/properties?service=${activeService}`
        );
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        const json = await res.json();

        const formatted: Property[] = (json.data || []).map((p: any) => {
          // prefer "actual"; if absent, fall back to min-max (formatted as range)
          const actual = p?.price?.actual;
          const min = p?.price?.min;
          const max = p?.price?.max;

          let priceText = "₹—";
          if (actual) {
            priceText = formatINRCompact(actual);
          } else if (min || max) {
            const left = min ? formatINRCompact(min) : "₹—";
            const right = max ? formatINRCompact(max) : "₹—";
            priceText = `${left} - ${right}`;
          }
    ...
    <section className="py-16 bg-secondary/30">
      <div className="container mx-auto px-4">
            ...
            {properties.map((property) => (
              ...
              >
                <div className="relative">
                  ...
                  />
                  <Badge
                    className={`absolute top-3 left-3 ${getServiceBadgeColor(
        ...
        <div className="text-center">
          ...
          >
            View All {activeService ? activeService.toLowerCase() : ""} Properties
            <ArrowRight className="h-4 w-4 ml-2" />
          </Button>
        </div>
      </div>
