...
function formatPrice(val?: string | number): string {
  if (typeof val === "number") return formatINRCompact(val);
  const n = Number(String(val ?? "").replace(/[^\d.]/g, "")); // strip ₹ , etc.
  return formatINRCompact(isNaN(n) ? undefined : n);
}
/* ----------------------------------------- */
            ...
            >
              <div className="relative">
                <img
                  ...
                  className="w-full h-48 object-cover"
                  onError={(e) => {
                    (e.currentTarget as HTMLImageElement).src = "/images/placeholder/property.jpg";
                  }}
                />
        ...
        <Dialog.Root open={!!currentVideo} onOpenChange={(open) => !open && setCurrentVideo(null)}>
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-11/12 md:w-3/4 lg:w-1/2 bg-background p-4 rounded shadow-lg z-50">
              ...
              {currentVideo && (
                <video
                  key={currentVideo}   // force reload on URL change
                  src={currentVideo}
                  controls
                  preload="metadata"
                  className="w-full h-[400px] object-cover rounded"
