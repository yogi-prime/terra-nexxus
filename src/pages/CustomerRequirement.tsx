...
// --- Map click hook (reverse geocode on click) ---
...
export default function CustomerRequirement() {
  ...
  // ✅ mapRef must be inside the component
  const mapRef = useRef<L.Map | null>(null);

  // debounced search to Nominatim (India-only)
  ...
  useEffect(() => {
    ...
    searchRef.current = window.setTimeout(async () => {
      try {
        ...
        const res = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${q}&countrycodes=in&limit=6&addressdetails=1`
        );
        const data = await res.json();
  ...
  // For country select: use iso2 as value (unique), but store dial in state
  const selectedCountryIso2 = useMemo(
    () => COUNTRIES.find((c) => c.dial === form.countryDial)?.iso2 ?? "IN",
    [form.countryDial]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isValidEmail = (v: string) =>
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim());
    const isValid10 = (v: string) => /^[0-9]{10}$/.test(v);
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 md:py-24 text-white">
          <div className="max-w-3xl">
            <p className="uppercase tracking-wider opacity-90 text-xs mb-2">
              Terranexxus • Concierge
            </p>
            <h1 className="text-4xl md:text-5xl font-bold leading-tight">
              Tell us your property requirement
            </h1>
            <p className="mt-3 text-white/90 max-w-2xl">
              Buy or lease—share your preferences, drop a pin on the map, and
              our experts will shortlist the best options.
            </p>
          </div>
        </div>
      </section>
                    ...
                    <div className="mt-1 flex gap-2">
                      <div className="w-40">
                        <Select
                          ...
                          onValueChange={(iso2) => {
                            const sel = COUNTRIES.find((c) => c.iso2 === iso2);
                            setForm((f) => ({
                              ...f,
                              countryDial: sel?.dial ?? "91",
                            }));
                          }}
                        >
                          <SelectTrigger>
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={
                            form.service ? "Select type" : "Choose service first"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <SelectContent>
                            {["None", "1 Slot", "2 Slots", "3+ Slots"].map(
                              (x) => (
                                <SelectItem key={x} value={x}>
                                  {x}
                                </SelectItem>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              carpetArea: e.target.value.replace(/\D/g, ""),
                            }))
                          }
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              superArea: e.target.value.replace(/\D/g, ""),
                            }))
                          }
                        />
                      </div>
                    ...
                    <>
                      <div>
                        ...
                        >
                          ...
                          <SelectContent>
                            {["11 months", "1 year", "3 years", "5 years"].map(
                              (x) => (
                                <SelectItem key={x} value={x}>
                                  {x}
                                </SelectItem>
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <Input
                      ...
                      value={form.areaText}
                      onChange={(e) =>
                        setForm((f) => ({ ...f, areaText: e.target.value }))
                      }
                    />
                  </div>
                  <div className="md:col-span-2">
                        ...
                        {searchResults.map((r, idx) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              // Move/zoom map to searched location
                    ...
                    <div className="rounded-xl overflow-hidden border h-[520px]">
                      ...
                      >
                        ...
                        <ClickCapture
                          onPick={(lat, lng, displayName) => {
                            setForm((f) => ({ ...f, lat, lng }));
                            if (displayName) setSearch(displayName);
                          }}
                        />
                        {form.lat && form.lng && (
